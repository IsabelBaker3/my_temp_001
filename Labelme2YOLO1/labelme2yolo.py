...
将labelme标注的json文件（矩形框的目标检测和分割）转换为yolo格式的txt文件，
制作成train和val两个文件夹，用于训练集和验证集。

要求：
图像和labelme标注的json文件必须放在同一目录下 

Usage:

python labelme2yolo.py -j <json_dir> -t <to_seg>

-j: the directory of labelme annotations.
-t: whether to convert to segmentation or not. (default: False)

The script will create a new directory named "YOLODataset_seg/" or "YOLODataset/" in the json_dir, which contains the converted annotations.
'''
import os
import sys
import argparse
...
class Labelme2YOLO(object):
    ...
    def _make_train_val_dir(self):
        self._train_dir_path = os.path.join(self._save_path_pfx, 'train/')
        self._val_dir_path = os.path.join(self._save_path_pfx, 'val/')

        for yolo_path in (os.path.join(self._train_dir_path + 'images/'), 
                          os.path.join(self._train_dir_path + 'labels/'), 
                          os.path.join(self._val_dir_path + 'images/'), 
                          os.path.join(self._val_dir_path + 'labels/')):
    ...
    def convert(self, val_size):
        ...
        for target_dir, json_names in zip(('train/', 'val/'), (train_json_names, val_json_names)):
            for json_name in json_names:
                ...
                img_path = self._save_yolo_image(json_name, target_dir)
    ...
    def _save_yolo_label(self, json_name, target_dir, yolo_obj_list):
        txt_path = os.path.join(self._save_path_pfx, target_dir, 'labels', json_name.replace('.json', '.txt'))
    ...
    def _save_yolo_image(self,json_name, target_dir):
        ...
        img_save_path = os.path.join(self._save_path_pfx, target_dir, 'images', img_name)
        
        if not os.path.exists(img_save_path):
            shutil.copyfile(os.path.join(self._json_dir, img_name), img_save_path)
    ...
    def _save_dataset_yaml(self):
        ...
        with open(yaml_path, 'w+') as yaml_file:
            yaml_file.write('# path to yolo dataset files \n')
            yaml_file.write('path: %s\n' % \
                            os.path.join(self._save_path_pfx))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_dir',type=str,default='G:/Data_Backup/CHAIR/data/project/Router/20271227/screw_check/2000x1500/aug/img_rename/',
                        help='Please input the path of the labelme json files.')
    parser.add_argument('--val_size',type=float, nargs='?', default=0.15,
